...
const API = "http://localhost:5000/api/cost-details";
...
function CostManager() {
  ...
  const editItem = (item) => {
    setForm({
      ...
      CostAccount: item["Cost Account"],
      CostDetails: item["COST Details"],
      Amount: item.Amount,
      PaymentDate: item["Payment Date"] ? item["Payment Date"].split("T")[0] : "",
    });
...
const handleSearch = () => {
  ...
  const filtered = data.filter(
    (row) =>
      ...
      (row["Cost Account"] && row["Cost Account"].toLowerCase().includes(lower))
  );
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead style={{ position: "sticky", top: 0, zIndex: 10 }}>
              <tr style={{ background: "#1976d2", color: "white" }}>
                {["S/N", "Date", "Location", "Cost Account", "COST Details", "Amount", "Payment Date", "Actions"].map(
                  (h) => (
                    <th key={h} style={head}>
                      {h}
                    </th>
            ...
            <tbody>
              {filteredData.map((row, i) => (
                <tr key={row.SN} style={{ background: i % 2 ? "#fff" : "#f3f7ff" }}>
                  ...
                  <td style={cell}>{row["Cost Account"]}</td>
                  <td style={cell}>{row["COST Details"]}</td>
                  <td style={cell}>{row.Amount}</td>
                  <td style={cell}>{row["Payment Date"] ? row["Payment Date"].split("T")[0] : ""}</td>
                  <td style={cell}>
